...
def test_analyze_data():
    ...
    }, index=pd.date_range('2021-06-01', periods=3, freq='T'))
    
    analysis = analyze_data(df)
    
    assert 'cpu_usage_mean' in analysis
    assert 'memory_usage_mean' in analysis
    assert 'disk_usage_mean' in analysis
    assert 'cpu_usage_max' in analysis
    assert 'memory_usage_max' in analysis
    assert 'disk_usage_max' in analysis
    assert analysis['cpu_usage_mean'] == 60.0
    assert analysis['cpu_usage_max'] == 70.0
